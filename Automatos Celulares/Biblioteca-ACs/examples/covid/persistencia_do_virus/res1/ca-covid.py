    ...
    def rule(self, x, y):
        ...
        if s == self.getOld(x, y):
            ...
            if s == VOID: 
                ...
                if the >= 1 and random() < 0.5:
                    return THE                 
                if tke >= 1 and random() < 0.5:
                    return TKE                 

                if i >= 1 or (apm >= 1 or the >= 1 or tke >= 1 and random() < 0.5):
                    return C 
            ...
            if s == TKN:
                if apm >= 1: # probabilidade de ativação aumenta com C 
                    return TKE 
            
            if s == TKE: 
                if random() < p_tke_death:
                    return VOID 
                if virus >= 1 and random() < 0.4:
                    return I 
...
c = CA_COVID(30, values=[0]*50 + [1]*1 + [2]*2 + [5]*2 + [7]*1)
...
plot(c, N=100, out='cacovid_result1.pdf',
    vmax=9, graphic=True,
